...
const Navbar = () => {
  ...
  return (
    <>
          ...
          >
            <div className={`flex items-center gap-1 px-2 py-2 rounded-full backdrop-blur-2xl transition-all duration-500 ${
              ...
              >
                ...
                <span className="text-sm font-semibold text-gray-900 tracking-wide font-mono">DV</span>
              </a>
                ...
                return (
                  <a
                    ...
                    className={`relative px-4 py-2 text-sm rounded-full transition-all duration-300 ${
                      isActive
                        ? "text-[#034694] bg-[#034694]/10 border border-[#034694]/30"
      ...
      {/* Mobile overlay */}
      {mobileOpen && (
        <div className="fixed inset-0 z-40 bg-white/95 backdrop-blur-2xl flex flex-col items-center justify-center gap-6 md:hidden">
          {navLinks.map((link, i) => (
            <a
              key={link.href}
              href={link.href}
